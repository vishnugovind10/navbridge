...
4. Define a versioned Policy Pack for the fund type and required evidence.
5. Validate administrator records with `navbridge validate-admin-file` or `validate_nav_records()`.
6. Run `navbridge batch` for a historical period and compare output against existing reconciliation breaks.
7. Write an audit manifest with `--audit-manifest` and store it with JSON and Markdown reports.
...
- Policy Packs: define reusable institutional thresholds, escalation expectations, and evidence requirements.
...
- Policy Pack versioning and approval workflow.
